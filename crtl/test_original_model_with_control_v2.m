...
%% Equations
    ...
    G = @(g0)[((VT-g0(2))-(Csa+Csv)*Pthorax)/(T+Beta/g0(5))-g0(1); ... f1
        ...
        Rs*g0(5)*g0(4)-g0(3);... f3
        (VT-g0(2))/(g0(5)*T+alpha)-g0(4);... f4
        ((Psa_star*g0(4))/(g0(3)*Vstroke_star))^lambda-(g0(5)/F_star);... f5
        ];
    ...
    J = @(g0)[-1,-1/(T+(alpha+(Csa+Csv)/CRVD)/g0(5)),0,0,(Beta*((VT-g0(2))-(Csa+Csv)*Pthorax))/(g0(5)*T+Beta)^2;...df1
        ...
        (lambda/g0(4))*(Psa_star*g0(4)/(g0(3)*Vstroke_star))^lambda,-1/F_star];
    ...
    for i = 1:length(g0)
        for j = 1:length(g0)
            ...
            fh = G(g0_star);
            f = G(g0);
            diffJ(i,j) = (f(j)-fh(j))./h;
        end
    end
    check = J(g0)-diffJ
    % we solve:
    [S,numIts] = mnewton(G,J,g0,100);
