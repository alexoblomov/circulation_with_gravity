...
% in L/mmHs
    ...
    Csv_original  = 0.00175; % systemic venous compliance (changed from 0.09 to be equal to arterial compliance)
...
%% control parameters
    Psa_U_star = 55624; %dynes/cm^2
    Vstroke_star = 0.07*L_to_cm3;
    F_star = F_original / 60; % in seconds
    Vsv0_L_star = (2/3)* Vsv0_original * L_to_cm3;  
%% model constants
    g = 980; %cm/s^2
    rho = 1; %g/cm^3
...
alpha = (Cpa+Cpv)/(CLVD);
T = Rs_U*Csa_U + Rp*Cpa;
K = Cpa*Rp+alpha; % constant term present in each of the three cases
    % the set of equations that describe the system
        G = @(g0)[lambda*(Vsv_L_star-g0(3))*((T*F_star*(g0(4)/Psa_U_star)+...
        alpha*((g0(5))/Vstroke_star))/(T*F_star+alpha)-1)-g0(3)+Vsv0_L_star;... 
        Rs_U*g0(6)*g0(5)-g0(4);...
        (VT-(Vsa0_U+Vsa0_L+Vsv0_U+g0(3)))/(g0(6)*(Rs*Csa+Rp*Cpa)+alpha)-g0(5);... 
        ((Psa_U_star*g0(5))/(g0(4)*Vstroke_star))^lambda-g0(6)/F_star];
    
    % the jacobian of the system
    J = @(g0)[-1,-(lambda*T)/Rs,lambda*alpha,0; % df3
        0,-1,Rs_U*g0(6),Rs_U*g0(5);... %df4
        -1/(g0(6)*T+alpha),0,-1,-((VT-(Vsa0_U-Vsa0_L-g0(3)-Vsa0_U))*T)/(g0(6)*T+alpha)^2;... % df5
        0,(-lambda/g0(4))*((Psa_U_star*g0(5))/(g0(4)*Vstroke_star))^lambda,...
    ...
    g0 = [Vsv0_L_star;Psa_U_star;Vstroke_star;F_star].*epsilon;
    
    % we solve:
    [S,numIts] = mnewton(G,J,g0,1000);
    
%% I. solutions for the control variables (stroke volume and heart rate)
% and their dependent variables(systemic arterial pressure:

Vstroke = S(3);% the stroke volume
F       = S(4);% the heart rate
Vsv0_L  = S(1);% the reserve volume of the systemic veins in the lower body
Psa_U   = S(2);% the systemic arterial pressure in the upper body
 
%% II. solutions for the other model variables: volumes, pressures in terms of the unkowns
% equations for the flows
