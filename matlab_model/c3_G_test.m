...
% holding arrays:
    solutions = zeros(length(case2_Pthorax_values),length(Earth_G_range),3); % initialize solutions array.
    % this holds the triplet: volume, upper flow and lower flow, for each case
    pressures = zeros(length(case2_Pthorax_values),length(Earth_G_range),4); % this holds the quartet of 
